    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultBjpk10:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultBjpk10:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultBjpk10:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultBjpk10:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultBjpk10List" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultBjpk10, getOpenresultBjpk10, delOpenresultBjpk10, addOpenresultBjpk10, updateOpenresultBjpk10 } from "@/api/lottery/openresultBjpk10";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultBjpk10(this.queryParams).then(response => {
        this.openresultBjpk10List = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultBjpk10(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultBjpk10(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除开奖结果(北京PK10)编号为"' + ids + '"的数据项？').then(function() {
