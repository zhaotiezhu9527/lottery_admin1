    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultGs1mft:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultGs1mft:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultGs1mft:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultGs1mft:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultGs1mftList" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultGs1mft, getOpenresultGs1mft, delOpenresultGs1mft, addOpenresultGs1mft, updateOpenresultGs1mft } from "@/api/lottery/openresultGs1mft";

export default {
  name: "OpenresultGs1mft",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultGs1mft(this.queryParams).then(response => {
        this.openresultGs1mftList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOpenresultGs1mft(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultGs1mft(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultGs1mft(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除1分飞艇开奖编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultGs1mft(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultGs1mft/export', {
        ...this.queryParams
      }, `openresultGs1mft_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
