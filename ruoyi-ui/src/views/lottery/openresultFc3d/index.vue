    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultFc3d:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultFc3d:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultFc3d:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultFc3d:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultFc3dList" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultFc3d, getOpenresultFc3d, delOpenresultFc3d, addOpenresultFc3d, updateOpenresultFc3d } from "@/api/lottery/openresultFc3d";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultFc3d(this.queryParams).then(response => {
        this.openresultFc3dList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加开奖结果(福彩3D)";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getOpenresultFc3d(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改开奖结果(福彩3D)";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultFc3d(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultFc3d(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除开奖结果(福彩3D)编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultFc3d(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultFc3d/export', {
        ...this.queryParams
      }, `openresultFc3d_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
