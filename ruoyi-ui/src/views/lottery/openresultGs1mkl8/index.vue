    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultGs1mkl8:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultGs1mkl8:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultGs1mkl8:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultGs1mkl8:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultGs1mkl8List" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultGs1mkl8, getOpenresultGs1mkl8, delOpenresultGs1mkl8, addOpenresultGs1mkl8, updateOpenresultGs1mkl8 } from "@/api/lottery/openresultGs1mkl8";

export default {
  name: "OpenresultGs1mkl8",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultGs1mkl8(this.queryParams).then(response => {
        this.openresultGs1mkl8List = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOpenresultGs1mkl8(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultGs1mkl8(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultGs1mkl8(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除1分快乐8编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultGs1mkl8(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultGs1mkl8/export', {
        ...this.queryParams
      }, `openresultGs1mkl8_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
