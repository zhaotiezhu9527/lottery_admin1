    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultGd11x5:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultGd11x5:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultGd11x5:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultGd11x5:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultGd11x5List" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultGd11x5, getOpenresultGd11x5, delOpenresultGd11x5, addOpenresultGd11x5, updateOpenresultGd11x5 } from "@/api/lottery/openresultGd11x5";

export default {
  name: "OpenresultGd11x5",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultGd11x5(this.queryParams).then(response => {
        this.openresultGd11x5List = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加开奖结果(广东11选5)";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getOpenresultGd11x5(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改开奖结果(广东11选5)";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultGd11x5(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultGd11x5(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除开奖结果(广东11选5)编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultGd11x5(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultGd11x5/export', {
        ...this.queryParams
      }, `openresultGd11x5_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
