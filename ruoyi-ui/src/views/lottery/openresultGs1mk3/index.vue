    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultGs1mk3:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultGs1mk3:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultGs1mk3List" @selection-change="handleSelectionChange">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultGs1mk3(this.queryParams).then(response => {
        this.openresultGs1mk3List = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加开奖结果(GS1分快3)";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getOpenresultGs1mk3(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改开奖结果(GS1分快3)";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultGs1mk3(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultGs1mk3(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除开奖结果(GS1分快3)编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultGs1mk3(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultGs1mk3/export', {
        ...this.queryParams
      }, `openresultGs1mk3_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
