<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="openresultHklhcList" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultHklhc, getOpenresultHklhc, delOpenresultHklhc, addOpenresultHklhc, updateOpenresultHklhc } from "@/api/lottery/openresultHklhc";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultHklhc(this.queryParams).then(response => {
        this.openresultHklhcList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultHklhc(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultHklhc(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除香港六合彩编号为"' + ids + '"的数据项？').then(function() {
