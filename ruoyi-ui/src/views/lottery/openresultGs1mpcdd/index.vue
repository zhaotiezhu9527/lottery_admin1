    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultGs1mpcdd:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultGs1mpcdd:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultGs1mpcdd:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultGs1mpcdd:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultGs1mpcddList" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultGs1mpcdd, getOpenresultGs1mpcdd, delOpenresultGs1mpcdd, addOpenresultGs1mpcdd, updateOpenresultGs1mpcdd } from "@/api/lottery/openresultGs1mpcdd";

export default {
  name: "OpenresultGs1mpcdd",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultGs1mpcdd(this.queryParams).then(response => {
        this.openresultGs1mpcddList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOpenresultGs1mpcdd(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultGs1mpcdd(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultGs1mpcdd(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除1分PC蛋蛋编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultGs1mpcdd(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultGs1mpcdd/export', {
        ...this.queryParams
      }, `openresultGs1mpcdd_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
