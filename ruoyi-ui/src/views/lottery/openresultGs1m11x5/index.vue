    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultGs1m11x5:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultGs1m11x5:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultGs1m11x5:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultGs1m11x5:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultGs1m11x5List" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultGs1m11x5, getOpenresultGs1m11x5, delOpenresultGs1m11x5, addOpenresultGs1m11x5, updateOpenresultGs1m11x5 } from "@/api/lottery/openresultGs1m11x5";

export default {
  name: "OpenresultGs1m11x5",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultGs1m11x5(this.queryParams).then(response => {
        this.openresultGs1m11x5List = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOpenresultGs1m11x5(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultGs1m11x5(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultGs1m11x5(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除1分11选5编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultGs1m11x5(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultGs1m11x5/export', {
        ...this.queryParams
      }, `openresultGs1m11x5_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
