    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultBjkl8:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultBjkl8:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultBjkl8:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultBjkl8:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultBjkl8List" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultBjkl8, getOpenresultBjkl8, delOpenresultBjkl8, addOpenresultBjkl8, updateOpenresultBjkl8 } from "@/api/lottery/openresultBjkl8";

export default {
  name: "OpenresultBjkl8",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultBjkl8(this.queryParams).then(response => {
        this.openresultBjkl8List = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加开奖结果(北京快乐8)";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getOpenresultBjkl8(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改开奖结果(北京快乐8)";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultBjkl8(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultBjkl8(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除开奖结果(北京快乐8)编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultBjkl8(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultBjkl8/export', {
        ...this.queryParams
      }, `openresultBjkl8_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
