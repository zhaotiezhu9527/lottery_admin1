    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultPcdd:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultPcdd:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultPcdd:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultPcdd:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultPcddList" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultPcdd, getOpenresultPcdd, delOpenresultPcdd, addOpenresultPcdd, updateOpenresultPcdd } from "@/api/lottery/openresultPcdd";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultPcdd(this.queryParams).then(response => {
        this.openresultPcddList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加开奖结果(PC蛋蛋)";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getOpenresultPcdd(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改开奖结果(PC蛋蛋)";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultPcdd(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultPcdd(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除开奖结果(PC蛋蛋)编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultPcdd(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultPcdd/export', {
        ...this.queryParams
      }, `openresultPcdd_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
