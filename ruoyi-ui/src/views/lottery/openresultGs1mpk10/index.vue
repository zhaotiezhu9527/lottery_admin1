<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultGs1mpk10:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="openresultGs1mpk10List" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultGs1mpk10, getOpenresultGs1mpk10, delOpenresultGs1mpk10, addOpenresultGs1mpk10, updateOpenresultGs1mpk10 } from "@/api/lottery/openresultGs1mpk10";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultGs1mpk10(this.queryParams).then(response => {
        this.openresultGs1mpk10List = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultGs1mpk10(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultGs1mpk10(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除开奖结果(GS1分PK10)编号为"' + ids + '"的数据项？').then(function() {
