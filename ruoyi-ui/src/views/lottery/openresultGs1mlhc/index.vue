    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultGs1mlhc:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['lottery:openresultGs1mlhc:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['lottery:openresultGs1mlhc:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultGs1mlhc:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="openresultGs1mlhcList" @selection-change="handleSelectionChange">
...
<script>
import { listOpenresultGs1mlhc, getOpenresultGs1mlhc, delOpenresultGs1mlhc, addOpenresultGs1mlhc, updateOpenresultGs1mlhc } from "@/api/lottery/openresultGs1mlhc";

export default {
  name: "OpenresultGs1mlhc",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultGs1mlhc(this.queryParams).then(response => {
        this.openresultGs1mlhcList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加1分六合彩开奖";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getOpenresultGs1mlhc(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改1分六合彩开奖";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultGs1mlhc(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultGs1mlhc(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除1分六合彩开奖编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultGs1mlhc(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultGs1mlhc/export', {
        ...this.queryParams
      }, `openresultGs1mlhc_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
