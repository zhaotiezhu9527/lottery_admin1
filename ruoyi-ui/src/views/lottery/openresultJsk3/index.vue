    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['lottery:openresultJsk3:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['lottery:openresultJsk3:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listOpenresultJsk3, getOpenresultJsk3, delOpenresultJsk3, addOpenresultJsk3, updateOpenresultJsk3 } from "@/api/lottery/openresultJsk3";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOpenresultJsk3(this.queryParams).then(response => {
        this.openresultJsk3List = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加开奖结果(江苏快3)";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getOpenresultJsk3(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改开奖结果(江苏快3)";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOpenresultJsk3(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOpenresultJsk3(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除开奖结果(江苏快3)编号为"' + ids + '"的数据项？').then(function() {
        return delOpenresultJsk3(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lottery/openresultJsk3/export', {
        ...this.queryParams
      }, `openresultJsk3_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
